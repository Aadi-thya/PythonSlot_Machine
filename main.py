...
def get_no_of_lines():
    while True:
        lines=input("Enter thr no of lines to bet on ( 1 - " +str(MAX_LINES)+ ") ? ")
        if lines.isdigit():
            lines=int(lines)
...
def get_bet():
    while True:
        amount=input("What would you like to bet on wach line ? $")
        if amount.isdigit():
            amount=int(amount)
...
def spins(balance):
    ...
    while True:
        ...
        if total_bet > balance :
            print(f"You do not have that much amunt to bed .Your current balace is ${balance}")
        else:
            break
